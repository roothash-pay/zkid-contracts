...
// Underlying architecture, Layer 1: BLSApkRegistry (public key registration and aggregation)
contract BLSApkRegistry is Initializable, OwnableUpgradeable, IBLSApkRegistry, BLSApkRegistryStorage, EIP712 {
    ...
    modifier onlyOracleManager() {
        require(msg.sender == oracleManager, "BLSApkRegistry.onlyOracleManager: caller is not the oracle manager address");
        _;
    }
    ...
    function initialize(address _initialOwner, address _whiteListAddress, address _oracleManager) external initializer {
    ...
    function registerBLSPublicKey(address operator, PubkeyRegistrationParams calldata params, BN254.G1Point calldata pubkeyRegistrationMessageHash) external returns (bytes32) {
        require(msg.sender == operator, "BLSApkRegistry.registerBLSPublicKey: this caller is not operator");

        require(blsRegisterWhitelist[msg.sender], "BLSApkRegistry.registerBLSPublicKey: this address have not permission to register bls key");

        bytes32 pubkeyHash = BN254.hashG1Point(params.pubkeyG1);

        require(pubkeyHash != ZERO_PK_HASH, "BLSApkRegistry.registerBLSPublicKey: cannot register zero pubkey");
        require(operatorToPubkeyHash[operator] == bytes32(0), "BLSApkRegistry.registerBLSPublicKey: operator already registered pubkey");

        require(pubkeyHashToOperator[pubkeyHash] == address(0), "BLSApkRegistry.registerBLSPublicKey: public key already registered");

        uint256 gamma = uint256(keccak256(abi.encodePacked(params.pubkeyRegistrationSignature.X, params.pubkeyRegistrationSignature.Y, params.pubkeyG1.X, params.pubkeyG1.Y, params.pubkeyG2.X, params.pubkeyG2.Y, pubkeyRegistrationMessageHash.X, pubkeyRegistrationMessageHash.Y))) % BN254.FR_MODULUS;

        require(BN254.pairing(params.pubkeyRegistrationSignature.plus(params.pubkeyG1.scalar_mul(gamma)), BN254.negGeneratorG2(), pubkeyRegistrationMessageHash.plus(BN254.generatorG1().scalar_mul(gamma)), params.pubkeyG2), "BLSApkRegistry.registerBLSPublicKey: either the G1 signature is wrong, or G1 and G2 private key do not match");
    ...
    function checkSignatures(bytes32 msgHash, uint256 referenceBlockNumber, NonSignerAndSignature memory params) public view returns (uint256, bytes32) {
        ...
        require(totalNodes - nonSingerNode >= thresholdNodes, "BLSSignatureChecker.checkSignatures: sign node less than threshold node");
    ...
    function trySignatureAndApkVerification(bytes32 msgHash, BN254.G1Point memory apk, BN254.G2Point memory apkG2, BN254.G1Point memory sigma) public view returns (bool pairingSuccessful, bool siganatureIsValid) {
        ...
        (pairingSuccessful, siganatureIsValid) = BN254.safePairing(sigma.plus(apk.scalar_mul(gamma)), BN254.negGeneratorG2(), BN254.hashToG1(msgHash).plus(BN254.generatorG1().scalar_mul(gamma)), apkG2, PAIRING_EQUALITY_CHECK_GAS);
    }
    ...
    function _processApkUpdate(BN254.G1Point memory point) internal {
        ...
        if (lastUpdate.updateBlockNumber == uint32(block.number)) {
            ...
            apkHistory.push(ApkUpdate({ apkHash: newApkHash, updateBlockNumber: uint32(block.number), nextUpdateBlockNumber: 0 }));
        }
    }
    ...
    function _initializeApk() internal {
        ...
        apkHistory.push(ApkUpdate({ apkHash: bytes24(0), updateBlockNumber: uint32(block.number), nextUpdateBlockNumber: 0 }));
    }
